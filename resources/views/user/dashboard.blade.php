...
@section('content')
    <div class="py-4 sm:py-6">
        ...
        <div class="mb-6 bg-gradient-to-r from-slate-800 via-slate-700 to-slate-800 rounded-xl shadow-lg p-4 sm:p-6">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-slate-300 text-sm">{{ \Carbon\Carbon::now()->locale('id')->isoFormat('dddd, D MMMM Y') }}
                    </p>
                </div>
        ...
        @if (session('success'))
            <div
                class="mb-4 p-4 bg-gradient-to-r from-emerald-50 to-emerald-50/50 border-l-4 border-emerald-500 rounded-lg shadow-sm">
        ...
        @if (session('warning'))
            <div
                class="mb-4 p-4 bg-gradient-to-r from-yellow-50 to-yellow-50/50 border-l-4 border-yellow-500 rounded-lg shadow-sm">
        ...
        @if ($schedule)
            @if ($schedule->is_validated)
                <div
                    class="bg-gradient-to-r from-blue-50 to-blue-50/50 border-l-4 border-blue-500 text-blue-800 px-4 py-3 rounded-lg mb-6 shadow-sm">
                    <div class="flex items-center">
                        ...
                        <p class="text-sm font-medium">Jadwal ini sudah divalidasi oleh admin. Tidak dapat melakukan
                            perubahan presensi.</p>
                    </div>
                </div>
            @endif
            ...
            <div class="bg-white shadow-lg rounded-xl p-4 sm:p-6 mb-4 sm:mb-6 border border-slate-200">
                <div class="border-b border-slate-200 pb-3 mb-4">
                    ...
                    <p class="text-sm text-slate-600 mt-1">{{ $schedule->day_name }},
                ...
                <div class="overflow-x-auto -mx-4 sm:mx-0">
                    <div class="inline-block min-w-full align-middle">
                        <div class="overflow-hidden border border-slate-200 rounded-lg">
                            <table class="min-w-full divide-y divide-slate-200">
                                <thead class="bg-gradient-to-r from-slate-50 to-slate-100">
                                    <tr>
                                        <th
                                            class="px-3 sm:px-6 py-3 text-center text-xs font-bold text-slate-700 uppercase tracking-wider">
                                            Nama</th>
                                        <th
                                            class="px-3 sm:px-6 py-3 text-center text-xs font-bold text-slate-700 uppercase tracking-wider">
                                            Status</th>
                                ...
                                <tbody class="bg-white divide-y divide-slate-100">
                                    @forelse ($schedule->persons as $person)
                                        ...
                                        <tr>
                                            ...
                                            <td
                                                class="px-3 sm:px-6 py-3 sm:py-4 whitespace-nowrap text-xs sm:text-sm text-gray-500 text-center">
                                                @if ($isCurrentUser)
                                                    @if (!$schedule->is_validated)
                        ...
                        <div class="mt-6">
                            ...
                            @if ($schedule->photo)
                                {{-- KONDISI 1: Foto sudah diupload --}}
                                <img src="{{ asset('storage/' . $schedule->photo) }}" alt="Foto Kehadiran"
                                    class="w-64 rounded shadow mb-3">
                            @elseif ($schedule->is_validated)
                                {{-- KONDISI 2: Belum ada foto, tapi jadwal SUDAH divalidasi (Form Terkunci/Hilang) --}}
                                <div
                                    class="bg-gray-50 p-6 rounded-xl border border-gray-200 border-dashed max-w-lg flex flex-col items-center justify-center text-center">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-gray-400 mb-2"
                                        fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                                    </svg>
                                    <p class="text-gray-500 font-medium text-sm">Upload foto dikunci.</p>
                                    <p class="text-gray-400 text-xs mt-1">Jadwal ini sudah divalidasi oleh admin.</p>
                                </div>
                            @else
                                {{-- KONDISI 3: Belum ada foto dan BELUM divalidasi (Tampilkan Form) --}}
                                <div class="bg-white p-6 rounded-xl shadow-md border border-gray-100 max-w-lg">
                            ...
                            border border-dashed border-gray-300 rounded-lg p-4 cursor-pointer">
                                            </label>

                                            <div class="flex justify-start">
                                                <button type="submit"
                                                    class="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-lg text-base font-bold transition duration-200 shadow-sm">
                                                    Simpan Foto
                                                </button>
                                            </div>
                                        </div>
            ...
            if (photoInput) {
                photoInput.addEventListener('change', function() {
                    ...
                    if (this.files[0] && this.files[0].size > maxFileSize) {
                        // Reset input file jadi kosong
                        this.value = '';

                        // Alert browser sederhana
                        alert('Maaf, ukuran foto terlalu besar. Maksimal 5 MB.');

                        // Atau jika ingin lebih canggih (opsional):
                        // Anda bisa memunculkan pesan error custom di HTML
                    }
                });
            }
        });
