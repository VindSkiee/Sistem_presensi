...
<body class="bg-gray-100">
    ...
    <div id="confirmModal" class="hidden fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4">
            <div class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm transition-opacity" onclick="closeConfirmModal()"></div>
            
            <div class="relative bg-white rounded-2xl shadow-2xl max-w-md w-full border border-slate-200 transform transition-all">
                <!-- Header -->
                <div class="bg-gradient-to-r from-slate-800 to-slate-700 px-6 py-4 rounded-t-2xl">
                    <h3 class="text-lg font-bold text-white flex items-center" id="modalTitle">
                        <svg class="w-6 h-6 mr-2" id="modalIcon" fill="none" stroke="currentColor" viewBox="0 0 24 24"></svg>
                ...
                <div class="bg-slate-50 px-6 py-4 rounded-b-2xl flex justify-end gap-3 border-t border-slate-200">
                    ...
                    <button id="modalConfirmBtn" 
                        class="px-4 py-2 font-medium rounded-lg transition-colors text-sm">
                    </button>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        function showConfirmModal(options) {
            ...
            if (options.type === 'danger') {
                ...
                confirmBtn.className = 'px-4 py-2 bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white font-medium rounded-lg transition-colors text-sm shadow-lg';
            } else if (options.type === 'warning') {
                icon.innerHTML = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path>';
                confirmBtn.className = 'px-4 py-2 bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-600 hover:to-yellow-700 text-white font-medium rounded-lg transition-colors text-sm shadow-lg';
            } else {
                ...
                confirmBtn.className = 'px-4 py-2 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-medium rounded-lg transition-colors text-sm shadow-lg';
            }
