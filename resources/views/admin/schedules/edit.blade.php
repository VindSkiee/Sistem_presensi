    ...
    <div class="bg-white shadow rounded-lg p-6">
        <form action="{{ route('admin.schedules.update', $schedule) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="date" name="date" id="date" value="{{ \Carbon\Carbon::parse($schedule->date)->format('d M Y') }}" required
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
                    @foreach($persons as $person)
                        <div class="flex items-center">
                            <input type="checkbox" name="persons[]" id="person_{{ $person->id }}" value="{{ $person->id }}" 
                                {{ $schedule->persons->contains($person->id) ? 'checked' : '' }}
                                class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded">
                            <label for="person_{{ $person->id }}" class="ml-2 text-sm text-gray-700">{{ $person->name }}</label>
                        </div>
                    @endforeach
                </div>
            </div>
